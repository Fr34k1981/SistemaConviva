...
def _df_estudantes_conselho(registro: dict) -> pd.DataFrame:
    ...
    cols = ["ra", "nome", "turma", "frequencia", "prova_paulista_1", "prova_paulista_2", "prova_interna", "dificuldades", "recomendacoes", "observacoes"]
...
<div style="margin:0 0 0.55rem 0;">
    <input disabled value="🔎 Acesso rápido" style="width:100%;height:34px;border:1px solid #d8e6f3;border-radius:2px;background:#fff;color:#64748b;padding:0 0.65rem;font-size:0.86rem;box-shadow:0 1px 4px rgba(15,23,42,0.06);" />
</div>
""", unsafe_allow_html=True)
...
# Cabeçalho global no padrão SED para páginas internas.
def _render_sed_header_global(pagina_atual: str):
    titulo_limpo = str(pagina_atual or "Dashboard")
    titulo_limpo = re.sub(r"^[^A-Za-zÀ-ÿ0-9]+", "", titulo_limpo).strip() or "Dashboard"
    usuario_nome = st.session_state.get("usuario_nome", "ALEANDRO FERREIRA DOS SANTOS")
    st.markdown(f"""
...
if menu != "🏠 Dashboard":
    _render_sed_header_global(menu)
...
# ======================================================
    ...
    page_header("Conselho de Classe", "Caderno online por turma: ata, protocolo, focos, pós-conselho e exportação", "#d946ef")

    st.markdown("""
    <div class="conselho-online-hero">
        <div class="conselho-online-title">🗂️ Conselho Online por Turma</div>
        <p class="conselho-online-sub">
            Preencha os documentos diretamente no sistema, com salvamento por turma e bimestre. A página foi organizada no mesmo espírito do Caderno de Tutoria: abas, histórico, campos editáveis e exportação em DOCX.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="conselho-form-note">
        ...
        Se a tabela <code>conselho_turmas</code> existir no Supabase, o sistema tenta salvar online. Se ainda não existir, salva no arquivo local <code>data/conselho_online_turmas.json</code>.
    </div>
    """, unsafe_allow_html=True)

    tab_visao, tab_ata, tab_perfil, tab_pec, tab_focos, tab_pos, tab_exportar = st.tabs([
        "🏠 Visão da turma",
        "📊 Ata e rendimento",
        "🧭 Perfil da turma",
        "✅ Protocolo PEC",
    ...
    with tab_visao:
        ...
        colv2.metric("Registros de rendimento", len(registro_conselho.get("estudantes", [])))
        ...
        st.markdown("""
        <div class="conselho-online-card">
            <h4>📌 Organização do Conselho</h4>
            <p>Use as abas para preencher cada parte do documento: ata, rendimento dos estudantes, perfil da turma, protocolo PEC, focos de atenção, pós-conselho e exportação.</p>
        </div>
        """, unsafe_allow_html=True)
        ...
        if st.button("🔄 Atualizar lista de estudantes desta turma", type="secondary", use_container_width=True, key="conselho_atualizar_lista"):
            existente = {str(e.get("ra") or e.get("nome", "")).strip(): e for e in registro_conselho.get("estudantes", [])}
            novos = []
            for _, row in alunos_turma_conselho.sort_values("nome").iterrows():
                chave_aluno = str(row.get("ra") or row.get("nome", "")).strip()
                item = existente.get(chave_aluno, {})
                item.update({
                    "ra": str(row.get("ra", "")).strip(),
                    "nome": str(row.get("nome", "")).strip(),
                    "turma": str(row.get("turma", turma_sel)).strip(),
                })
                for c in ["frequencia", "prova_paulista_1", "prova_paulista_2", "prova_interna", "dificuldades", "recomendacoes", "observacoes"]:
                    item.setdefault(c, "")
                novos.append(item)
            registro_conselho["estudantes"] = novos
            _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
            st.success("Lista de estudantes atualizada no Conselho da turma.")
            st.rerun()
    ...
    with tab_ata:
        ...
        with col_a1:
            registro_conselho["data"] = str(st.date_input("Data do Conselho", value=pd.to_datetime(registro_conselho.get("data", datetime.now().strftime("%Y-%m-%d"))).date(), key="conselho_data"))
        with col_a2:
            registro_conselho["horario"] = st.text_input("Horário", value=registro_conselho.get("horario", ""), key="conselho_horario")
        with col_a3:
            registro_conselho["local"] = st.text_input("Local", value=registro_conselho.get("local", "Sala de Informática"), key="conselho_local")
        registro_conselho["participantes"] = st.text_input("Participantes", value=registro_conselho.get("participantes", "Direção, CGPG, CGPACs e professores da turma"), key="conselho_participantes")
        ...
        st.caption("Preencha frequência, avaliações, dificuldades e recomendações. Use os códigos 1 a 6 para dificuldades e 7 a 12 para recomendações, conforme a legenda.")
        ...
        edit_est = st.data_editor(
            ...
            column_config={
                ...
                "prova_paulista_1": st.column_config.TextColumn("Prova Paulista 1"),
        ...
        with st.expander("📖 Legenda de dificuldades e recomendações", expanded=False):
            ...
            with col_l1:
                ...
                for cod, desc in DIFICULDADES_CONSELHO.items():
                    st.write(f"{cod} – {desc}")
            with col_l2:
                st.markdown("**Recomendações / providências**")
                for cod, desc in RECOMENDACOES_CONSELHO.items():
                    st.write(f"{cod} – {desc}")

        if st.button("💾 Salvar ata e rendimento", type="primary", use_container_width=True, key="salvar_ata_conselho"):
            _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
            st.success("Ata e rendimento salvos para esta turma.")
    ...
    with tab_perfil:
        ...
        perfil["dificuldades_aprendizagem"] = st.multiselect("Os estudantes apresentam dificuldades na aprendizagem?", [
            ...
            "Não conseguem acompanhar a aula, sendo necessário retomar conteúdos",
            "Apresentam defasagem de habilidades e precisam de atividades diferenciadas",
            "Outro",
        ], default=perfil.get("dificuldades_aprendizagem", []), key="perfil_dificuldades")
        perfil["comportamento"] = st.multiselect("Qual o comportamento geral apresentado pela turma?", [
            "Turma majoritariamente bem-comportada e respeitosa",
            "Há casos isolados de indisciplina que precisam de monitoramento",
            "Necessita medidas mais firmes quanto à indisciplina e respeito",
            "Apresenta bom comportamento",
            "Outro",
        ], default=perfil.get("comportamento", []), key="perfil_comportamento")
        perfil["participacao"] = st.multiselect("Como a turma participa na aula?", [
            "Participação ativa, com interesse e engajamento",
            "Alguns estudantes são tímidos e precisam ser incentivados",
            "Participação baixa, com muitos estudantes que não realizam atividades",
            "Outro",
        ], default=perfil.get("participacao", []), key="perfil_participacao")
        perfil["relacoes"] = st.multiselect("Como são as relações interpessoais?", [
            "Bom relacionamento entre os estudantes, com poucos conflitos",
            "Espírito de colaboração e ajuda mútua",
            "Há conflitos e dificuldade de convivência",
            "Não há colaboração entre eles",
            "Outro",
        ], default=perfil.get("relacoes", []), key="perfil_relacoes")
        ...
        perfil["recomendacoes"] = st.multiselect("Recomendações", [
            ...
            "Outra",
        ], default=perfil.get("recomendacoes", []), key="perfil_recomendacoes")
        ...
        if st.button("💾 Salvar perfil da turma", type="primary", use_container_width=True, key="salvar_perfil_conselho"):
            _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
            st.success("Perfil da turma salvo.")

    with tab_pec:
        st.markdown("### ✅ Protocolo de Acompanhamento do Conselho — PEC")
        st.caption("Marque se cada elemento foi evidenciado, não evidenciado ou evidenciado em partes.")
        ...
        for idx, item in enumerate(protocolo):
            with st.expander(f"Item {item.get('item', idx+1)} — {item.get('elemento', '')[:80]}", expanded=False):
                ...
                status = st.radio(
                    "Situação",
                    ["Evidenciado", "Não evidenciado", "Evidenciado em partes"],
                    index=(["Evidenciado", "Não evidenciado", "Evidenciado em partes"].index(item.get("status")) if item.get("status") in ["Evidenciado", "Não evidenciado", "Evidenciado em partes"] else 0),
                    horizontal=True,
                    key=f"pec_status_{idx}",
                )
        ...
        if st.button("💾 Salvar protocolo PEC", type="primary", use_container_width=True, key="salvar_pec_conselho"):
            _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
            st.success("Protocolo PEC salvo.")

    with tab_focos:
        st.markdown("### 🎯 Focos de atenção")
        st.caption("Registre estudantes apontados no Conselho, frequência, notas abaixo de cinco e presença/ciência.")
        if st.button("✨ Sugerir focos a partir dos estudantes da ata", type="secondary", use_container_width=True, key="sugerir_focos_conselho"):
            ...
            for est in registro_conselho.get("estudantes", []):
                freq_txt = str(est.get("frequencia", "")).replace("%", "").replace(",", ".").strip()
                try:
                    freq_val = float(freq_txt) if freq_txt else 100
                except Exception:
                    freq_val = 100
                dif = str(est.get("dificuldades", "")).strip()
                rec = str(est.get("recomendacoes", "")).strip()
                if freq_val < 85 or dif or rec:
                    focos.append({
                        "serie": est.get("turma", turma_sel),
                        "estudante": est.get("nome", ""),
                        "frequencia": est.get("frequencia", ""),
                        "notas_abaixo": "",
                        "presenca": "",
                    })
            registro_conselho["focos"] = focos
            st.success("Lista de focos sugerida. Revise antes de salvar.")
        ...
        if st.button("💾 Salvar focos de atenção", type="primary", use_container_width=True, key="salvar_focos_conselho"):
            _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
            st.success("Focos de atenção salvos.")
    ...
    with tab_pos:
        ...
        if st.button("📋 Copiar focos para o pós-conselho", type="secondary", use_container_width=True, key="copiar_focos_pos"):
            registro_conselho["pos_conselho"] = [
                {"serie": f.get("serie", ""), "estudante": f.get("estudante", ""), "frequencia": f.get("frequencia", ""), "notas_abaixo": f.get("notas_abaixo", ""), "ciencia": f.get("presenca", "")}
                for f in registro_conselho.get("focos", [])
            ]
        ...
        if st.button("💾 Salvar pós-conselho", type="primary", use_container_width=True, key="salvar_pos_conselho"):
            _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
            st.success("Pós-conselho salvo.")

    with tab_exportar:
        st.markdown("### 🖨️ Exportar e modelos originais")
        col_e1, col_e2 = st.columns(2)
        with col_e1:
            if st.button("💾 Salvar tudo antes de exportar", type="primary", use_container_width=True, key="salvar_tudo_conselho"):
                _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
                st.success("Conselho salvo.")
            docx_bytes = _gerar_docx_conselho_turma(registro_conselho)
            if docx_bytes:
                nome_docx = f"conselho_{_normalizar_chave_conselho(turma_sel)}_{_normalizar_chave_conselho(bimestre_sel)}_{ano_letivo_sel}.docx"
                st.download_button(
                    "⬇️ Baixar Conselho preenchido em DOCX",
                    data=docx_bytes,
                    file_name=nome_docx,
                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                    use_container_width=True,
                    key="baixar_docx_conselho_preenchido",
                )
            st.download_button(
                "⬇️ Baixar dados em JSON",
                data=json.dumps(registro_conselho, ensure_ascii=False, indent=2).encode("utf-8"),
                file_name=f"dados_{chave_conselho}.json",
                mime="application/json",
                use_container_width=True,
                key="baixar_json_conselho",
            )
        with col_e2:
            ...
            for d in DOCUMENTOS_CONSELHO_PADRAO:
                ...
                if caminho.exists():
                    st.download_button(
                        ...
                        mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        key=f"download_modelo_original_{d['arquivo']}",
                        use_container_width=True,
                    )

    # salvamento silencioso local a cada render para preservar alterações na sessão
    _salvar_registro_conselho(chave_conselho, registro_conselho, base_conselho)
